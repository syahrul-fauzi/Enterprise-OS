...
export const createCase = {
    ...
    async execute(input) {
        ...
        const { title, description, priority, sessionId, sourceDiscussionId, id: preferredId, decision_id, last_invocation_digest, ptEstablishmentDetails, trainingDetails, linkedIntentId } = parsed;
        ...
        if (parsed.tenantId && parsed.workspaceId && parsed.actorId) {
            ...
            if (parsed.tenantId && parsed.workspaceId && parsed.actorId) {
                ...
                if (isAnonymous) {
                    ...
                    const session = await sessionRepository.byId(sessionId);
        ...
        if (preferredId !== undefined) {
            const existing = await caseRepository.byId(preferredId, { tenantId, workspaceId });
            if (existing)
                throw new Error(`[case.create] Preferred case ID already exists: ${preferredId}`);
            caseId = preferredId;
        }
        const entity = {
            id: caseId,
...
export const closeCase = {
    ...
    async execute(input) {
        ...
        const current = await caseRepository.byId(id, {
            tenantId: tenantId,
            workspaceId: workspaceId
        });
        ...
        const next = { ...current, status: "closed", closedAt, workId: current.workId };
...
export const markCaseCompleted = {
    kind: "command",
