...
import type { WorkParticipant, WorkRealityPerspective } from './work-reality.types';
...
export function PeopleSection({ participants, currentPerspective, workId }: PeopleSectionProps) {
  ...
  const canAdd = currentPerspective === 'operator' || currentPerspective === 'professional' || currentPerspective === 'agent' || currentPerspective === 'notary';
  ...
  const handleAdd = async () => {
    if (!workId || !newName.trim() || sending) return;
    setSending(true);
    setError(null);
    try {
      const participantId = `${newRole}-${newName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-001`;
      const res = await fetch("/api/communication/send", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          work_id: workId,
          content: `[EOS] ${roleLabelMap[newRole]} ditambahkan ke Work: ${newName} (${participantId}).`,
          recipient_ids: [participantId]
        })
      });
      if (!res.ok) throw new Error(`Gagal menambahkan (${res.status})`);
      setNewName('');
      setNewRole('professional');
      setShowAdd(false);
      window.location.reload();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Gagal menambahkan partisipan');
    } finally {
      setSending(false);
    }
  };
