...
export type Arch15AViolationKind = "env-var-read" | "filesystem-access" | "network-request" | "global-singleton-access" | "react-context-access" | "local-storage-read" | "non-deterministic-date-now" | "non-deterministic-math-random" | "non-deterministic-performance-now" | "source-contract-violation";
export type Arch15BViolationKind = "graph-hash-mismatch" | "plan-hash-mismatch" | "normalized-hash-mismatch" | "normalized-canonical-json-mismatch" | "plan-canonical-hash-mismatch" | "plan-canonical-json-mismatch" | "graph-canonical-json-mismatch" | "graph-structural-checksum-mismatch" | "normalized-id-mismatch" | "node-ordering-mismatch" | "region-ordering-mismatch" | "slot-ordering-mismatch" | "navigation-ordering-mismatch" | "by-kind-count-mismatch" | "active-capabilities-ordering-mismatch" | "resolved-snapshot-mismatch";
...
export type VerifyArch15AFn = (source: DescriptorSource, resolver: Partial<ResolverContext> & {
    readonly capabilityEntries?: Readonly<Record<string, ResolverCapabilityEntry>>;
}, options?: {
    readonly strict?: boolean;
}) => Arch15APurityReport;
export type VerifyArch15BFn = (source: DescriptorSource, resolver: Partial<ResolverContext> & {
    readonly capabilityEntries?: Readonly<Record<string, ResolverCapabilityEntry>>;
}, options?: {
    readonly iterations?: number;
    readonly strict?: boolean;
}) => Arch15BStabilityReport;
export type VerifyArch15Fn = (source: DescriptorSource, resolver: Partial<ResolverContext> & {
    readonly capabilityEntries?: Readonly<Record<string, ResolverCapabilityEntry>>;
}, options?: {
    readonly iterations?: number;
    readonly strict?: boolean;
