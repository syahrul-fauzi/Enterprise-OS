...
exports.verifyArch15 = exports.verifyArch15B = exports.verifyArch15A = void 0;
exports.snapshotFromResult = snapshotFromResult;
exports.composeWithArch15 = composeWithArch15;
const compose_1 = require("../compose");
function hashInput(source, resolver) {
    let h = 2166136261 >>> 0;
    const feed = (s) => {
...
function dependencyAuditComposePath() {
    const allowedSources = ["descriptor", "kernel-contracts", "capability-registry", "resolver-context"];
    const forbiddenSources = ["env-var", "filesystem", "network", "global-singleton", "react", "local-storage"];
    const allowedSideEffectDomains = ["durationMs", "profiling", "diagnostics", "fitness-reporting"];
    const forbiddenSideEffectDomains = ["cache-key", "node-id", "ordering", "canonical-json", "plan-id", "graph-hash", "structural-checksum", "composition-hash"];
    const details = [];
...
const verifyArch15A = function verifyArch15A(source, resolver, options) {
    ...
    const allDescriptorFields = ["workspace", "layoutRegistry", "regionRegistry", "slotRegistry", "navigationRegistry"];
    const descriptorFields = allDescriptorFields.filter(f => f in source);
    const resolverContextFields = ["actor", "features"];
    if (resolver?.capabilityEntries !== undefined)
        resolverContextFields.push("capabilityEntries");
    if (resolver?.slotOverrides !== undefined)
        resolverContextFields.push("slotOverrides");
    if (resolver?.requestId !== undefined)
        resolverContextFields.push("requestId");
    const forbiddenSideEffectSourcePresent = {
        "process.env": false,
        "fs (filesystem)": false,
...
exports.verifyArch15A = verifyArch15A;
const verifyArch15B = function verifyArch15B(source, resolver, options) {
    const t0 = typeof performance !== "undefined" && typeof performance.now === "function" ? performance.now() : Date.now();
    const iterations = Math.max(2, Math.min(32, options?.iterations ?? 5));
    const strict = options?.strict ?? true;
    const violations = [];
    const inputHash = hashInput(source, resolver);
    const composeInput = { ...source, resolver };
    const snapshots = [];
    const planHashes = [];
    const graphHashes = [];
    const normalizedHashes = [];
    const planCanonicalHashes = [];
    const graphStructuralChecksums = [];
    const normalizedCanonicalJsons = [];
    const planCanonicalJsons = [];
    const graphCanonicalJsons = [];
    const resolvedActiveCapabilityOrders = [];
    const regionOrders = [];
    const slotOrders = [];
    const navigationOrders = [];
    const nodeOrders = [];
    for (let i = 0; i < iterations; i++) {
        const r = (0, compose_1.compose)(composeInput);
    ...
    const allStable = normalizedHashStable &&
...
exports.verifyArch15B = verifyArch15B;
const verifyArch15 = function verifyArch15(source, resolver, options) {
    const t0 = typeof performance !== "undefined" && typeof performance.now === "function" ? performance.now() : Date.now();
    const purity = (0, exports.verifyArch15A)(source, resolver, { strict: options?.strict });
    const stability = (0, exports.verifyArch15B)(source, resolver, { iterations: options?.iterations, strict: options?.strict });
    const violations = [...purity.violations, ...stability.violations];
    const t1 = typeof performance !== "undefined" && typeof performance.now === "function" ? performance.now() : Date.now();
    return {
        overall: {
            rule: "ARCH-15",
...
exports.verifyArch15 = verifyArch15;
function composeWithArch15(input, options) {
    const result = (0, compose_1.compose)(input);
    if (options?.arch15 === undefined)
        return { compose: result };
    const resolver = input.resolver ?? {};
    const report = (0, exports.verifyArch15)({
