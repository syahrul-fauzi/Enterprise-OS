...
exports.ALPHA_6_CLAIMS = exports.EVIDENCE_ID_KEYS = void 0;
exports.buildCertificationMatrix = buildCertificationMatrix;
exports.alpha6Matrix = alpha6Matrix;
exports.claimsSupportedBy = claimsSupportedBy;
exports.claimsThatSupport = claimsThatSupport;
exports.claimDependencies = claimDependencies;
exports.claimDependents = claimDependents;
exports.claimsForEvidenceId = claimsForEvidenceId;
exports.contradictors = contradictors;
exports.superseding = superseding;
exports.relationsSummary = relationsSummary;
exports.claimLineage = claimLineage;
exports.dependencyClosure = dependencyClosure;
exports.evidenceRevocationImpact = evidenceRevocationImpact;
exports.runCertificationSelfTest = runCertificationSelfTest;
exports.compareCertificationSnapshots = compareCertificationSnapshots;
exports.computeFullRevocationCascade = computeFullRevocationCascade;
const types_1 = require("./types");
const evidence_1 = require("./evidence");
function countByLevelAndStatus(claims) {
    const base = {
        Execution: {
            PASS: 0,
            FAIL: 0,
...
function listExecutionResolved(claims) {
    return Object.values(claims)
        .filter(c => c.evidenceLevel === "Execution" && (c.status === "PASS" || c.status === "FAIL"))
        .map(c => c.id)
        .sort();
}
function listExecutionUnresolved(claims) {
    return Object.values(claims)
        .filter(c => c.evidenceLevel === "Execution" && c.status !== "PASS" && c.status !== "FAIL")
        .map(c => c.id)
        .sort();
}
function listArchitecturalHypotheses(claims) {
    return Object.values(claims)
        .filter(c => c.evidenceLevel === "Architectural")
        .map(c => c.id)
        .sort();
}
function listEvolutionaryClaims(claims) {
    return Object.values(claims)
...
const ALPHA6_EVIDENCE_PKGS = Object.freeze({
...
function buildEvidenceIdentityIndex(pkgs, derivationParentKeys = {}) {
    const phase1 = {};
    for (const [key, pkg] of Object.entries(pkgs)) {
        phase1[key] = (0, evidence_1.computeEvidenceIdSync)(pkg);
    }
    ...
    if (needsRebuild.length === 0)
        return Object.freeze(phase1);
    const validParentKeys = {};
    for (const k of needsRebuild)
        validParentKeys[k] = (derivationParentKeys[k] ?? []).filter(p => Object.prototype.hasOwnProperty.call(pkgs, p));
    const inDeg = {};
    for (const k of needsRebuild)
        inDeg[k] = 0;
    const children = {};
    for (const k of needsRebuild) {
        for (const p of validParentKeys[k]) {
            if (!Object.prototype.hasOwnProperty.call(validParentKeys, p))
                continue;
            if (!children[p])
                children[p] = [];
            children[p].push(k);
    ...
    const final = { ...phase1 };
    const keyToEid = {};
    for (const [key, ident] of Object.entries(final))
        keyToEid[key] = ident.id;
    for (const key of order) {
        ...
        const rebuilt = Object.freeze({
            ...
            derivedFromEvidenceIds: Object.freeze([
                ...(base.derivedFromEvidenceIds ?? []),
                ...parentEids,
            ]),
        });
        const newIdent = (0, evidence_1.computeEvidenceIdSync)(rebuilt);
        final[key] = newIdent;
        keyToEid[key] = newIdent.id;
    }
    return Object.freeze(final);
}
const ALPHA6_DERIVATION_PARENTS = Object.freeze({
...
const ALPHA6_EVIDENCE_IDS = buildEvidenceIdentityIndex(ALPHA6_EVIDENCE_PKGS, ALPHA6_DERIVATION_PARENTS);
exports.EVIDENCE_ID_KEYS = Object.freeze({
...
const evId = (key) => Object.freeze([ALPHA6_EVIDENCE_IDS[exports.EVIDENCE_ID_KEYS[key]].id]);
const ALPHA6_CLAIM_RELATIONS = Object.freeze([
    Object.freeze({
        fromClaimId: "eos.compiler.cross-process.determinism.3-pid",
        kind: "supports",
        toClaimId: "eos.compiler.purity.audit-no-forbidden-domain",
        rationale: "Cross-process determinism (3 distinct PIDs) memperkuat observasi bahwa tidak ada hidden side-effect domain yang mempengaruhi output pada kondisi yang diuji.",
    }),
    Object.freeze({
        fromClaimId: "eos.compiler.arch15.determinism.14dim",
        kind: "supports",
        toClaimId: "eos.compiler.purity.audit-no-forbidden-domain",
        rationale: "14-dimensional stable output dalam single-process repeated run memperkuat bahwa tidak ada mutable singleton atau non-deterministic source yang terpicu antar run cepat.",
    }),
    Object.freeze({
        fromClaimId: "eos.compiler.immutable-ir",
        kind: "supports",
        toClaimId: "eos.compiler.arch15.determinism.14dim",
        rationale: "IR deep-immutability mencegah accidental mutation di dalam pipeline yang dapat merusak determinisme hash.",
    }),
    Object.freeze({
        fromClaimId: "eos.compiler.hashing.canonical",
        kind: "supports",
        toClaimId: "eos.compiler.arch15.determinism.14dim",
        rationale: "Hash derived dari canonical serialization menjamin bahwa nilai hash tidak berubah hanya karena traversal order — bagian dari stable output ARCH-15B.",
    }),
    Object.freeze({
        fromClaimId: "eos.compiler.canonical-serialization.6-type-reject",
        kind: "supports",
        toClaimId: "eos.compiler.hashing.canonical",
        rationale: "Penolakan eksplisit Function/Symbol/WeakMap/dll memastikan canonical source untuk hashing tidak mengandung value yang berpotensi menghasilkan string berbeda tiap run.",
    }),
    Object.freeze({
        fromClaimId: "eos.compiler.arch16.fitness-falsifiability",
        kind: "supports",
        toClaimId: "a7.runtime.boundary.only-consumes-resolved-workspace",
        rationale: "ARCH-16 fitness falsifiability membuktikan bahwa verification mechanism (verifyArch16) mampu membedakan patuh vs tidak patuh — ini adalah prasyarat agar claim Runtime Boundary Hypothesis dapat dinyatakan Supported atau Refuted.",
    }),
    Object.freeze({
        fromClaimId: "eos.compiler.arch16.constitution",
        kind: "supports",
        toClaimId: "a7.runtime.boundary.only-consumes-resolved-workspace",
        rationale: "ARCH-16 Constitution mendefinisikan secara formal SPESIFIKASI apa yang harus dipenuhi Runtime Boundary. Tanpa spesifikasi ini, hypothesis tak dapat difalsifikasi.",
    }),
    Object.freeze({
        fromClaimId: "a7.runtime.boundary.only-consumes-resolved-workspace",
        kind: "dependsOn",
        toClaimId: "a7.runtime.package-created",
        rationale: "Architectural boundary hypothesis hanya dapat diuji (status Supported/Refuted) ketika runtime package fisik sudah ada sebagai Execution evidence dan source-nya dapat discan oleh verifyArch16().",
    }),
    Object.freeze({
        fromClaimId: "a7.runtime.cannot-influence-planning-output",
        kind: "dependsOn",
        toClaimId: "a7.runtime.package-created",
        rationale: "Experiment EXP-A7-PLAN-IMMUTABILITY membutuhkan package runtime fisik untuk menerapkan capability filter dan membandingkan hash planning.",
    }),
    Object.freeze({
        fromClaimId: "evo.capability-addition.zero-runtime-changes",
        kind: "dependsOn",
        toClaimId: "a7.runtime.boundary.only-consumes-resolved-workspace",
        rationale: "Kalau boundary runtime bahkan belum dapat dipastikan Supported, maka zero-runtime-change adalah property yang tidak dapat diukur secara evolusioner.",
    }),
    Object.freeze({
        fromClaimId: "evo.capability-based-planning.alignment",
        kind: "dependsOn",
        toClaimId: "evo.capability-addition.zero-runtime-changes",
        rationale: "Capability-Based Planning alignment membutuhkan rangkaian penambahan capability; claim zero-runtime-change adalah langkah pertama pembuktian CBP.",
    }),
    Object.freeze({
        fromClaimId: "evo.product-composition.multi-workspace",
        kind: "dependsOn",
        toClaimId: "a7.runtime.boundary.only-consumes-resolved-workspace",
        rationale: "Multi-workspace composition membutuhkan runtime yang berperilaku konsisten per boundary — yaitu hanya mengonsumsi ResolvedWorkspace.",
    }),
    Object.freeze({
        fromClaimId: "eos.compiler.graph-builder.interface-fplan",
        kind: "supports",
        toClaimId: "a7.runtime.boundary.only-consumes-resolved-workspace",
        rationale: "Interface buildGraphFromPlan hanya menerima CompositionPlan → secara struktural graph construction stage terisolasi dari runtime host layer.",
    }),
    Object.freeze({
        fromClaimId: "eos.compiler.referential-equivalence.3-entry",
        kind: "supports",
        toClaimId: "eos.compiler.arch15.determinism.14dim",
        rationale: "3 entry-point equivalen menunjukkan bahwa 5-stage pipeline bersih dan determinisme tidak tergantung pada entry-point mana yang digunakan.",
    }),
    Object.freeze({
        fromClaimId: "eos.compiler.ci.gates.4-stack",
        kind: "supports",
        toClaimId: "eos.compiler.purity.audit-no-forbidden-domain",
        rationale: "CI 4-gate (type/lint/build/self-test) memberikan guarantee reproduktifitas bahwa seluruh Execution Evidence dijalankan dalam repository state yang bersih dan type-safe.",
    }),
]);
exports.ALPHA_6_CLAIMS = Object.freeze({
    "eos.compiler.immutable-ir": {
        id: "eos.compiler.immutable-ir",
        title: "Intermediate Representations are deep-immutable value objects",
        description: "NormalizedWorkspace, CompositionPlan, WorkspaceGraph seluruhnya melewati Object.freeze rekursif. Strict-mode assignment ke instance melempar TypeError.",
        evidenceLevel: "Execution",
        gate: "Architecture",
        status: "PASS",
        evidenceIds: evId("IR_IMMUTABILITY"),
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_IR_IMMUTABILITY,
    ...
    "eos.compiler.canonical-serialization.6-type-reject": {
        ...
        description: "Tidak ada silent coerce. Seluruh 6 kategori terlarang melempar CanonicalSerializationError dengan `kind` dan `path` yang dapat diaudit.",
        evidenceLevel: "Execution",
        gate: "Architecture",
        status: "PASS",
        evidenceIds: evId("CANONICAL_REJECT"),
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_CANONICAL_REJECT_6TYPES,
        ownerMilestone: "alpha.6",
    },
    "eos.compiler.hashing.canonical": {
        ...
        description: "Normalized.hash, CompositionPlan.canonicalHash, WorkspaceGraph.hash seluruhnya dihitung sebagai fnv1a32(canonicalSerialize(ir_unfrozen)). Hash bukan berasal dari object traversal biasa.",
        evidenceLevel: "Execution",
        gate: "Architecture",
        status: "PASS",
        evidenceIds: evId("HASH_DERIVED"),
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_HASH_CANONICAL_DERIVED,
        ownerMilestone: "alpha.6",
    },
    "eos.compiler.referential-equivalence.3-entry": {
        ...
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_3ENTRY_EQUIV,
        ownerMilestone: "alpha.6",
    },
    "eos.compiler.graph-builder.interface-fplan": {
        ...
        description: "Signature buildGraphFromPlan hanya menerima parameter CompositionPlan. Tidak ada parameter Registry/Runtime/Clock di interface.",
        evidenceLevel: "Execution",
        gate: "Architecture",
        status: "PASS",
        evidenceIds: evId("IFACE_AUDIT"),
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_INTERFACE_STATIC_AUDIT,
        ownerMilestone: "alpha.6",
    },
    "eos.compiler.purity.audit-no-forbidden-domain": {
        ...
        description: "Assessment methodology: grep audit + repeated single-process determinism (ARCH-15B) + 3-process cross determinism. Claim ini menggambarkan observasi pada kondisi yang diuji, BUKAN mathematical purity proof.",
        evidenceLevel: "Execution",
        gate: "Architecture",
        status: "PASS",
        evidenceIds: evId("PURITY_AUDIT"),
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_PURITY_AUDIT,
        ownerMilestone: "alpha.6",
    },
    "eos.compiler.arch15.determinism.14dim": {
        ...
        description: "ARCH-15A Pure Composition violations=0. ARCH-15B Stability 14 flag seluruhnya stable=true pada 5 iterations (normalizedHash, normalizedCanonicalJson, planCanonicalHash, planCanonicalJson, planHash, graphHash, structuralChecksum, graphCanonicalJson, region, slot, navigation, node ordering, activeCapabilities, snapshotEquality).",
        evidenceLevel: "Execution",
        gate: "Security",
        status: "PASS",
        evidenceIds: evId("ARCH15"),
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_ARCH15_14DIM,
        ownerMilestone: "alpha.6",
    },
    "eos.compiler.arch16.constitution": {
        ...
        description: "Constitution: compilerProduces=[ResolvedWorkspace]; runtimeOnlyConsumes=[ResolvedWorkspace]; runtimeForbiddenStages=[normalization, planning, graph-construction, descriptor-interpretation]. Wording package-name-agnostik.",
        evidenceLevel: "Execution",
        gate: "Architecture",
        status: "PASS",
        evidenceIds: evId("ARCH16_CONSTITUTION"),
        specification: {
            specification: "Runtime hanya menerima ResolvedWorkspace sebagai input. Runtime TIDAK AKAN memanggil normalizeWorkspace / buildCompositionPlan / buildGraph / melakukan descriptor interpretation dalam bentuk apapun.",
            verificationMechanism: "verifyArch16(target) — static dependency graph audit dari glob runtime modules vs glob compiler modules.",
    ...
    "eos.compiler.arch16.fitness-falsifiability": {
        ...
        description: "Scenario positive (clean runtime module paths) menghasilkan verifyArch16 passed=true. Scenario negative (runtime menyentuh buildgraph) menghasilkan passed=false dengan violations>0.",
        evidenceLevel: "Execution",
        gate: "Architecture",
        status: "PASS",
        evidenceIds: evId("ARCH16_FALSIFY"),
        specification: {
            specification: "verifyArch16() harus dapat membedakan antara runtime yang patuh constitution (passed=true) dan yang melanggar (passed=false + violations). Fitness function TIDAK BOLEH selalu hijau.",
    ...
    "eos.compiler.ci.gates.4-stack": {
        ...
        description: "Seluruh 4 exit code 0. Lint dengan max-warnings 0 (bukan max-warnings 999). Self-test 14/14 assertions PASS.",
        evidenceLevel: "Execution",
        gate: "Repository",
        status: "PASS",
        evidenceIds: evId("CI4STACK"),
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_CI_4STACK,
        ownerMilestone: "alpha.6",
    },
    "eos.compiler.cross-process.determinism.3-pid": {
        ...
        description: "PID berbeda = proses OS berbeda. 8 hash fields (normalizedHash, planId, planCanonicalHash, planCanonicalJsonLen, graphHash, graphStructuralChecksum, graphCanonicalJsonLen, composeHash) identik. activeCapabilityIds dan nodeCount identik. 23/23 assertions PASS.",
        evidenceLevel: "Execution",
        gate: "Architecture",
        status: "PASS",
        evidenceIds: evId("CROSS3PID"),
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_CROSS_PROCESS_3PID,
        ownerMilestone: "alpha.6",
    },
    "a7.runtime.package-created": {
        ...
        provenance: ALPHA6_EVIDENCE_PKGS.PKG_A7_RUNTIME_PACKAGE_EXISTS,
    ...
    "a7.runtime.boundary.only-consumes-resolved-workspace": {
        ...
        description: "Mount signature: mount(resolved: ResolvedWorkspace, hostEnv?): RuntimeMountResult. Runtime TIDAK DAPAT memanggil normalizeWorkspace/buildCompositionPlan/buildGraph/interpret descriptor dalam bentuk apapun. Dipastikan via verifyArch16 strict=true pada target @repo/core-runtime.",
        evidenceLevel: "Architectural",
        gate: "Architecture",
        status: "Supported",
        specification: {
            specification: "@repo/core-runtime package hanya mengimport ResolvedWorkspace type dari @repo/composition. SELURUH stage normalizeWorkspace, buildCompositionPlan, buildGraph, descriptor kernel interpretation — TIDAK TERSEDIA di import graph runtime package.",
            verificationMechanism: "verifyArch16({ runtimeModulePaths: @repo/core-runtime source files, compilerModulePaths: @repo/composition full tree }) strict=true. Claim Supported jika passed=true && violations.length === 0 && seluruh forbiddenStages === false.",
    ...
    "a7.runtime.cannot-influence-planning-output": {
        ...
        description: "Planning (normalize + plan + graph) diisolasi: output deterministik sepenuhnya dari descriptor-source. Runtime slot-override hanya diperbolehkan pada ResolvedWorkspace, dan bila berubah → planning hash tetap, hanya resolved yang berubah.",
        evidenceLevel: "Architectural",
        gate: "Architecture",
        status: "Pending",
        specification: {
            specification: "Override apapun pada runtime layer (filter capability, slot override, permissions) TIDAK BOLEH mengubah CompositionPlan.canonicalHash atau WorkspaceGraph.hash. Hash planning immutable setelah pipeline berjalan.",
            verificationMechanism: "Test: build plan → apply runtime capability filter → rebuild resolved, assert plan.canonicalHash === original.",
    ...
    "evo.capability-addition.zero-runtime-changes": {
        ...
        description: "Evolutionary property. Simulasikan penambahan minimal 2 capability (contoh: legal-payment + legal-client). Validate: diff @repo/runtime/src === empty. Bukti boundary memberikan nilai evolusioner.",
    ...
    "evo.capability-based-planning.alignment": {
        ...
        description: "Apakah arsitektur EOS mendukung prinsip CBP: capability adalah unit perencanaan, capability dapat ditambahkan secara independen, dan runtime tidak mengetahui detail domain kapabilitas. Ini adalah property evolusi sistem, bukan property compiler.",
        evidenceLevel: "Evolutionary",
        gate: "Platform",
        status: "Planned",
        ownerMilestone: "beta.1",
    ...
    "evo.product-composition.multi-workspace": {
        ...
        description: "Evolutionary property. 2 workspace berbeda (lawyers-hub-pro + lawyers-hub-enterprise) masing-masing dengan capability set berbeda. Bukti compiler scales tanpa runtime mutation.",
        evidenceLevel: "Evolutionary",
        status: "Planned",
        ownerMilestone: "alpha.10",
    },
});
function buildCertificationMatrix(milestone, extraClaims = {}, extraEvidence = {}, extraRelations = [], extraProvenanceRegistry, extraExtendedPackages, extraDefinitionPairs) {
    const mergedClaims = { ...exports.ALPHA_6_CLAIMS };
    for (const k of Object.keys(extraClaims))
        mergedClaims[k] = extraClaims[k];
    Object.freeze(mergedClaims);
    const mergedPkgs = { ...ALPHA6_EVIDENCE_PKGS };
    for (const k of Object.keys(extraEvidence))
        mergedPkgs[k] = extraEvidence[k];
    const mergedDerivationParents = { ...ALPHA6_DERIVATION_PARENTS };
    ...
    const idNormalizedRelations = rawRelations.map(r => {
        if (r.id)
            return r;
        const { id } = (0, evidence_1.computeRelationIdSync)(r);
    ...
    for (const ident of Object.values(mergedEvidence)) {
        evIdToPkg[ident.id] = ident.pkg;
    }
    const mutableClaims = { ...mergedClaims };
    for (const [cid, claim] of Object.entries(mutableClaims)) {
        if (claim.evidenceLevel !== "Execution" || claim.status !== "PASS")
            continue;
        if (claim.observedEvidence &&
    ...
    const graphTopo = (0, evidence_1.computeGraphTopologyIdSync)(finalClaims, relations);
    const provenanceGraph = extraProvenanceRegistry
        ? (() => {
            ...
            for (const extPkg of Object.values(extraExtendedPackages ?? {})) {
                ...
                const id = (0, evidence_1.computeEvidenceIdSync)(extPkg);
            ...
            for (const [pkgKey, ident] of Object.entries(mergedEvidence)) {
                ...
                if (keyed) {
                    extByPkgKey[pkgKey] = keyed;
                    continue;
                }
                const byId = extById[String(ident.id)];
                if (byId)
                    extByPkgKey[pkgKey] = byId;
            }
            return (0, evidence_1.buildProvenanceGraph)({
    ...
    const envelopeBase = Object.freeze({
        protocolVersion: "1.0",
        epistemicProtocolVersion: types_1.EPISTEMIC_PROTOCOL_VERSION,
        evidenceSchemaVersion: types_1.EVIDENCE_SCHEMA_VERSION,
        relationLayerRules: types_1.RELATION_LAYER_RULES,
        evidenceLayers: types_1.EVIDENCE_LAYER_DEFINITION,
        layerLifecycle: types_1.LAYER_LIFECYCLE_STATUS,
        layerStatusSemantics: types_1.LAYER_STATUS_SEMANTICS,
        producedAt,
    ...
    const snapshot = (0, evidence_1.computeSnapshotIdSync)(envelopeBase);
    const envelope = Object.freeze({ ...envelopeBase, snapshotId: snapshot.id });
    ...
    if (!testReport.passed) {
        const messages = testReport.results
            ...
            .join("; ");
        throw new Error(`Certification Matrix Self-Test FAILED for milestone=${milestone}: ${messages}`);
    }
    return envelope;
}
function alpha6Matrix(producedClaims) {
    return buildCertificationMatrix("alpha.6", producedClaims);
}
function claimsSupportedBy(matrix, claimId) {
    const ids = new Set();
    for (const rel of matrix.claimRelations) {
        if (rel.fromClaimId === claimId && rel.kind === "supports")
            ids.add(rel.toClaimId);
    }
    return Object.values(matrix.claims)
        .filter(c => ids.has(c.id))
        .sort((a, b) => a.id.localeCompare(b.id));
}
function claimsThatSupport(matrix, claimId) {
    const ids = new Set();
    for (const rel of matrix.claimRelations) {
        if (rel.toClaimId === claimId && rel.kind === "supports")
            ids.add(rel.fromClaimId);
    }
    return Object.values(matrix.claims)
        .filter(c => ids.has(c.id))
        .sort((a, b) => a.id.localeCompare(b.id));
}
function claimDependencies(matrix, claimId) {
    const ids = new Set();
    for (const rel of matrix.claimRelations) {
        if (rel.fromClaimId === claimId && rel.kind === "dependsOn")
            ids.add(rel.toClaimId);
    }
    return Object.values(matrix.claims)
        .filter(c => ids.has(c.id))
...
function claimDependents(matrix, claimId) {
    const ids = new Set();
    for (const rel of matrix.claimRelations) {
        if (rel.toClaimId === claimId && rel.kind === "dependsOn")
            ids.add(rel.fromClaimId);
    }
    return Object.values(matrix.claims)
        .filter(c => ids.has(c.id))
...
function claimsForEvidenceId(matrix, evidenceId) {
    return Object.values(matrix.claims)
        .filter(c => (c.evidenceIds ?? []).includes(evidenceId))
        .sort((a, b) => a.id.localeCompare(b.id));
}
function contradictors(matrix, claimId) {
    return matrix.claimRelations.filter(r => (r.fromClaimId === claimId || r.toClaimId === claimId) && r.kind === "contradicts");
}
function superseding(matrix, claimId) {
    return matrix.claimRelations.filter(r => (r.fromClaimId === claimId || r.toClaimId === claimId) && r.kind === "supersedes");
}
function relationsSummary(matrix) {
    const base = { supports: 0, dependsOn: 0, contradicts: 0, supersedes: 0 };
    for (const r of matrix.claimRelations)
        base[r.kind] = (base[r.kind] ?? 0) + 1;
    return Object.freeze({ ...base });
}
function claimLineage(matrix, claimId) {
    const claim = matrix.claims[claimId];
    if (!claim)
        return null;
    const threats = claim.threatsToValidity ?? [];
    const supportsOutgoing = [];
    const supportsIncoming = [];
    const dependsOnOutgoing = [];
    const dependsOnIncoming = [];
    const relationIds = [];
    for (const rel of matrix.claimRelations) {
        ...
        if (rel.fromClaimId === claimId) {
            if (rel.kind === "supports")
                supportsOutgoing.push(rel.toClaimId);
            if (rel.kind === "dependsOn")
                dependsOnOutgoing.push(rel.toClaimId);
        }
        if (rel.toClaimId === claimId) {
            if (rel.kind === "supports")
                supportsIncoming.push(rel.fromClaimId);
            if (rel.kind === "dependsOn")
                dependsOnIncoming.push(rel.fromClaimId);
        }
    }
    const futureClaimsUnlocked = [];
    for (const downId of supportsOutgoing)
        futureClaimsUnlocked.push(downId);
    for (const rel of matrix.claimRelations) {
        if (rel.kind !== "dependsOn")
            continue;
        if (rel.toClaimId !== claimId)
...
function dependencyClosure(matrix, claimId) {
    const claim = matrix.claims[claimId];
    if (!claim)
        return null;
    const adj = {};
    for (const r of matrix.claimRelations) {
        if (!adj[r.fromClaimId])
            adj[r.fromClaimId] = [];
        if (r.kind === "dependsOn" || r.kind === "supports")
    ...
    while (stack.length > 0) {
        ...
        for (const nxt of adj[cur] ?? []) {
            if (visited.has(nxt))
                continue;
            visited.add(nxt);
            edges.push(Object.freeze([cur, nxt]));
            stack.push(nxt);
        }
    }
    return Object.freeze({
    ...
    for (const ident of Object.values(matrix.evidencePackages)) {
        ...
        while (visit.length > 0 && !found) {
            ...
            if (seen.has(asStr))
                continue;
            seen.add(asStr);
            if (String(eid) === String(evidenceId)) {
                found = true;
                break;
            }
            const ident2 = Object.values(matrix.evidencePackages).find(x => String(x.id) === asStr);
            if (ident2)
                for (const p2 of ident2.pkg.derivedFromEvidenceIds ?? [])
                    visit.push(p2);
        }
        if (found)
            descendantEvidence.push(ident.id);
    }
    return Object.freeze({
...
const EXECUTION_STATUS_STRENGTH = Object.freeze({
    "Not-Yet-Evaluated": -1, "FAIL": 0, "PASS": 1,
});
const ARCHITECTURAL_STATUS_STRENGTH = Object.freeze({
    "Not-Yet-Evaluated": -1, "Pending": 0, "Refuted": 0, "Supported": 1,
});
const EVOLUTIONARY_STATUS_STRENGTH = Object.freeze({
    "Not-Yet-Evaluated": -1, "Planned": 0, "Refuted": 0, "Running": 1, "Verified": 2,
});
function statusStrength(evidenceLevel, status) {
...
function findOrphanClaims(claims, relations) {
...
function findDuplicateRelations(relations) {
    const seen = new Set();
    const dupes = [];
    for (const r of relations) {
...
function findContradictoryPairs(claims, relations) {
...
function findInvalidLayerRelations(claims, relations) {
    const bad = [];
    for (const r of relations) {
        ...
        if (!from || !to)
            continue;
        if (!(0, types_1.isRelationAllowed)(from.evidenceLevel, r.kind, to.evidenceLevel)) {
            bad.push(`${r.kind}: ${r.fromClaimId}(${from.evidenceLevel}) → ${r.toClaimId}(${to.evidenceLevel})`);
        }
    }
    return Object.freeze(bad.sort());
}
function runCertificationSelfTest(input) {
    const { claims, evidencePackages, claimRelations, envelope } = input;
    const results = [];
    const evidenceIdToPkgKey = {};
    for (const [pkgKey, ident] of Object.entries(evidencePackages)) {
        evidenceIdToPkgKey[ident.id] = pkgKey;
    }
    const invClaimEvidence = [];
    for (const claim of Object.values(claims)) {
        const eids = claim.evidenceIds ?? [];
        for (const eid of eids) {
            const found = evidenceIdToPkgKey[eid];
            if (!found) {
                invClaimEvidence.push(`${claim.id} references unknown evidenceId ${String(eid).slice(0, 20)}...`);
                continue;
            }
            const pkgIdent = evidencePackages[found];
            const reverseClaims = Object.values(claims).filter(c => (c.evidenceIds ?? []).includes(pkgIdent.id));
            if (!reverseClaims.some(c => c.id === claim.id)) {
                invClaimEvidence.push(`reverse-lookup broken: claim ${claim.id} → pkg ${found} tidak reverse-mengandung claim`);
            }
        }
    }
    results.push({
        ...
        message: invClaimEvidence.length === 0
    ...
    const invEvidenceVerif = [];
    for (const [pkgKey, ident] of Object.entries(evidencePackages)) {
        const v = (0, evidence_1.verifyEvidenceIdentity)(ident);
        if (!v.ok) {
            invEvidenceVerif.push(`${pkgKey}: expected=${v.expected} recomputed=${v.recomputedId}`);
        }
    }
    results.push({
        id: "INV_EVIDENCE_ID_VERIFIABLE",
        passed: invEvidenceVerif.length === 0,
        message: invEvidenceVerif.length === 0
    ...
    const archMinSupports = [];
    for (const claim of Object.values(claims)) {
        if (claim.evidenceLevel !== "Architectural")
            continue;
        if (claim.status === "Pending")
            continue;
        const hasSupports = claimRelations.some(r => r.toClaimId === claim.id && r.kind === "supports");
        if (!hasSupports)
            archMinSupports.push(`${claim.id} (status=${claim.status})`);
    }
    results.push({
        id: "INV_ARCHITECTURAL_MIN_1_SUPPORTS",
        passed: archMinSupports.length === 0,
        message: archMinSupports.length === 0
    ...
    const execPassNoObserved = [];
    for (const claim of Object.values(claims)) {
        if (claim.evidenceLevel !== "Execution")
            continue;
    ...
    results.push({
        ...
        message: execPassNoObserved.length === 0
        ...
        details: execPassNoObserved.length > 0 ? Object.freeze(execPassNoObserved) : undefined,
    });
    const allRelationEndpointsMissing = [];
    ...
    results.push({
        ...
        message: allRelationEndpointsMissing.length === 0
    ...
    for (const ident of Object.values(evidencePackages))
        evidenceIdSet.add(ident.id);
    const derivedWithoutParents = [];
    const derivedUnknownParent = [];
    for (const [pkgKey, ident] of Object.entries(evidencePackages)) {
        const p = ident.pkg;
        if (p.derivation === "Raw")
            continue;
        ...
        for (const parId of parents) {
            if (!evidenceIdSet.has(parId)) {
                derivedUnknownParent.push(`${pkgKey} (derivation=${p.derivation}) references unknown parent ${String(parId).slice(0, 20)}…`);
            }
        }
    }
    ...
    results.push({
        ...
        message: derivationIssues.length === 0
    ...
    const relationMissingId = [];
    const relationIdBad = [];
    const relationIdCollisions = {};
    for (const r of claimRelations) {
        const key = `${r.fromClaimId} --[${r.kind}]--> ${r.toClaimId}`;
        if (!r.id) {
        ...
        const v = (0, evidence_1.verifyRelationIdentity)(r);
        if (!v.ok) {
            relationIdBad.push(`${key}: expected=${String(v.expected).slice(0, 20)}… recomputed=${String(v.recomputedId).slice(0, 20)}…`);
        }
        const asStr = String(r.id);
        if (!relationIdCollisions[asStr])
            relationIdCollisions[asStr] = [];
        relationIdCollisions[asStr].push(key);
    }
    const collisions = [];
    for (const [rid, rels] of Object.entries(relationIdCollisions)) {
        if (rels.length > 1)
            collisions.push(`${rid.slice(0, 24)}… shared by ${rels.length} relations: ${rels.join(", ")}`);
    }
    results.push({
        id: "INV_RELATION_IDS_ASSIGNED",
        passed: relationMissingId.length === 0,
        message: relationMissingId.length === 0
    ...
    results.push({
        ...
        message: relationIdBad.length === 0
    ...
    const execEids = new Set();
    for (const claim of Object.values(claims)) {
        if (claim.evidenceLevel !== "Execution")
            continue;
        for (const eid of claim.evidenceIds ?? [])
            execEids.add(String(eid));
    }
    const statusTransNoDelta = [];
    for (const claim of Object.values(claims)) {
        ...
        if (claim.evidenceLevel === "Architectural" && claim.status === "Supported" && newEids.length === 0) {
            statusTransNoDelta.push(`${claim.id} (Architectural Supported): SELURUH evidenceIds (${eids.length}) juga dirujuk oleh Execution claim lain. ΔEvidenceId = 0 — status naik Pending→Supported HARUS didampingi Evidence ID BARU.`);
        }
        if (claim.evidenceLevel === "Evolutionary" && claim.status === "Verified" && newEids.length === 0) {
            statusTransNoDelta.push(`${claim.id} (Evolutionary Verified): evidenceIds (${eids.length}) tidak ada yang unik vs Execution claims — status naik ke Verified HARUS memiliki Evidence ID BARU.`);
        }
    }
    results.push({
        id: "INV_STATUS_TRANSITION_REQUIRES_NEW_EVIDENCE",
        passed: statusTransNoDelta.length === 0,
        message: statusTransNoDelta.length === 0
    ...
    results.push({
        ...
        message: threatNoMitigation.length === 0
        ...
        details: threatNoMitigation.length > 0 ? Object.freeze(threatNoMitigation) : undefined,
    });
    ...
    results.push({
        ...
        message: invalidLayers.length === 0
    ...
    results.push({
        ...
        message: contradictories.length === 0
    ...
    if (envelope) {
        ...
        if (!envelope.snapshotId)
            snapMissing.push("envelope.snapshotId undefined atau null");
        results.push({
        ...
        const snapBad = [];
        if (envelope.snapshotId) {
            const ver = (0, evidence_1.verifySnapshotIdentity)(envelope);
            if (!ver.ok) {
                snapBad.push(`snapshotId mismatch: expected=${String(ver.expected).slice(0, 20)}… recomputed=${String(ver.recomputedId).slice(0, 20)}…`);
            }
        }
        results.push({
    ...
    const provenanceMissingNodes = [];
    const provenanceBadPrefix = [];
    const provenanceEmptyObservations = [];
    const provenanceObservationCountMismatch = [];
    for (const [pkgKey, ident] of Object.entries(evidencePackages)) {
        const p = ident.pkg;
        const prov = p.provenance;
        if (!prov)
            continue;
        if (!prov.experimentDefinitionId ||
        ...
        if (!defId.startsWith("exd:sha256:"))
            provenanceBadPrefix.push(`${pkgKey}: experimentDefinitionId prefix harus 'exd:sha256:', got '${defId.slice(0, 12)}…'`);
        if (!exeId.startsWith("exe:sha256:"))
            provenanceBadPrefix.push(`${pkgKey}: experimentExecutionId prefix harus 'exe:sha256:', got '${exeId.slice(0, 12)}…'`);
        for (let i = 0; i < obsIds.length; i++) {
            if (!obsIds[i].startsWith("obs:sha256:"))
                provenanceBadPrefix.push(`${pkgKey}: rawObservationIds[${i}] prefix harus 'obs:sha256:', got '${obsIds[i].slice(0, 12)}…'`);
        }
            ...
            if (obsIds.length !== p.rawObservations.length) {
                if (pkgKey.startsWith("__")) {
                    ...
                    if (!allSharedResolvableInRegistry) {
                        provenanceObservationCountMismatch.push(`${pkgKey}: provenance.rawObservationIds.length=${obsIds.length} > pkg.rawObservations.length=${p.rawObservations.length} DAN ${unresolved.length} shared references TIDAK TERDAFTAR di envelope.provenanceRegistry.rawObservations. Unresolved: ${unresolved.slice(0, 5).join(", ")}.`);
                    }
                }
                else {
                    provenanceObservationCountMismatch.push(`${pkgKey}: provenance.rawObservationIds.length=${obsIds.length} < pkg.rawObservations.length=${p.rawObservations.length} (beberapa observasi string TIDAK PUNYA identity provenance — count harus 1:1 untuk OWN observations).`);
                }
            }
        }
    }
    ...
    const provenanceDiverseExecutions = [];
    const exdToExeMap = {};
    for (const [pkgKey, ident] of Object.entries(evidencePackages)) {
        ...
        if (!exdToExeMap[defKey])
            exdToExeMap[defKey] = [];
        if (!exdToExeMap[defKey].includes(exeKey))
            exdToExeMap[defKey].push(exeKey);
    }
    void provenanceDiverseExecutions;
    const provenanceNoSharedExecBetweenDifferentExperiments = [];
    const exeUsedByPkgKeys = {};
    for (const [pkgKey, ident] of Object.entries(evidencePackages)) {
        const prov = ident.pkg.provenance;
        if (!prov)
            continue;
        const exeIdStr = String(prov.experimentExecutionId);
        if (!exeUsedByPkgKeys[exeIdStr])
            exeUsedByPkgKeys[exeIdStr] = [];
    ...
    for (const [exeIdStr, pkgKeys] of Object.entries(exeUsedByPkgKeys)) {
        if (pkgKeys.length > 1) {
            ...
            for (const pk of pkgKeys) {
                ...
                if (eid)
                    distinctExperiments.add(eid.pkg.experimentId);
            }
            if (distinctExperiments.size > 1) {
                provenanceNoSharedExecBetweenDifferentExperiments.push(`Execution ${exeIdStr.slice(0, 16)}… digunakan oleh experimentId BERBEDA: ${Array.from(distinctExperiments).join(" vs ")} — Satu experimentExecutionId harus berasal dari SATU experimentDefinitionId dan SATU experimentId.`);
            }
        }
    }
    results.push({
    ...
    for (const [obsIdStr, pkgKeys] of Object.entries(obsIdUsed)) {
        if (pkgKeys.length > 1) {
            ...
            if (!graphReuseValid) {
                provenanceObsUniqueness.push(`RawObservationId ${obsIdStr.slice(0, 16)}… dipakai oleh ${pkgKeys.length} packages BERBEDA (${pkgKeys.join(", ")}) TAPI TIDAK TERDAFTAR di registry.rawObservations — SHA-256 collision / dangling reference / provenance mapping error (GRAPH MODEL VALIDATION: obs harus terdaftar di registry jika dishare).`);
            }
        }
    }
    results.push({
    ...
    if (registryPresent) {
        const registryDefIdsRecomputable = [];
        for (const [defKey, entry] of Object.entries(registry.experimentDefinitions)) {
            const recomputed = (0, evidence_1.computeExperimentDefinitionIdSync)(entry.def);
            if (String(recomputed.id) !== defKey) {
                registryDefIdsRecomputable.push(`ExperimentDefinitionId ${defKey.slice(0, 18)}… MISMATCH. recomputedSHA256=${String(recomputed.id).slice(0, 18)}… — identity registry entry tidak konsisten SHA-256(canonical(def)) dengan stored ID.`);
            }
        }
        results.push({
        ...
        const registryExeIdsRecomputable = [];
        for (const [exeKey, entry] of Object.entries(registry.experimentExecutions)) {
            const recomputed = (0, evidence_1.computeExperimentExecutionIdSync)(entry.exe);
            if (String(recomputed.id) !== exeKey) {
                registryExeIdsRecomputable.push(`ExperimentExecutionId ${exeKey.slice(0, 18)}… MISMATCH. recomputedSHA256=${String(recomputed.id).slice(0, 18)}… — SHA256(canonical(execution)) !== stored execution.id (provenance chain integrity).`);
            }
        }
        results.push({
        ...
        const registryObsIdsRecomputable = [];
        for (const [obsKey, entry] of Object.entries(registry.rawObservations)) {
            const recomputed = (0, evidence_1.computeRawObservationIdSync)(entry.obs);
            if (String(recomputed.id) !== obsKey) {
                registryObsIdsRecomputable.push(`RawObservationId ${obsKey.slice(0, 18)}… MISMATCH. recomputedSHA256=${String(recomputed.id).slice(0, 18)}… — SHA256(canonical(observation)) !== stored observation.id (EPIDEMIOLOGIS: ini persis kondisi yang user ragukan: apakah obs:sha256 benar dari SHA256(canonical(obs)) atau sekadar generator ID).`);
            }
        }
        results.push({
        ...
        const registryPkgObsIdsInRegistry = [];
        ...
        for (const [pkgKey, ident] of Object.entries(evidencePackages)) {
            ...
            for (const oid of prov.rawObservationIds) {
                if (!rawObsRegistryKeys.has(String(oid))) {
                    registryPkgObsIdsInRegistry.push(`Package ${pkgKey} → provenance.rawObservationIds[${String(oid).slice(0, 18)}…] TIDAK TERDAFTAR dalam envelope.provenanceRegistry.rawObservations. Auditor tidak bisa merecompute SHA-256 karena object RawObservation hilang.`);
                }
            }
        }
        results.push({
    ...
    if (graph) {
        const reuseIdx = (0, evidence_1.computeObservationReuseIndex)(graph);
        const semCounts = (0, evidence_1.countSemanticEvidenceEdges)(graph);
        ...
        for (const [ekey, e] of Object.entries(graph.evidenceObservationEdges)) {
            if (!allEvidenceIds.has(String(e.fromEvidenceId)))
                evEdgeIntegrity.push(`edge ${ekey.slice(0, 18)}…: fromEvidenceId=${String(e.fromEvidenceId).slice(0, 18)}… tidak ada di evidencePackages identity index`);
            if (!allObsIds.has(String(e.toRawObservationId)))
                evEdgeIntegrity.push(`edge ${ekey.slice(0, 18)}…: toRawObservationId=${String(e.toRawObservationId).slice(0, 18)}… tidak ada di registry rawObservations`);
        }
        results.push({
        ...
        if (verEdges.length > 0) {
            ...
            for (const [k, e] of Object.entries(graph.definitionVersionLineageEdges)) {
                if (!registry?.experimentDefinitions[String(e.newDefinitionId)])
                    problems.push(`version edge ${k.slice(0, 18)}… newDefinitionId ${String(e.newDefinitionId).slice(0, 18)}… tidak terdaftar di registry.experimentDefinitions`);
                if (!registry?.experimentDefinitions[String(e.supersedesDefinitionId)])
                    problems.push(`version edge ${k.slice(0, 18)}… supersedesDefinitionId ${String(e.supersedesDefinitionId).slice(0, 18)}… tidak terdaftar di registry.experimentDefinitions`);
            }
            results.push({
...
function compareCertificationSnapshots(oldEnv, newEnv) {
    const idA = (0, evidence_1.computeSnapshotIdSync)(oldEnv).id;
    const idB = (0, evidence_1.computeSnapshotIdSync)(newEnv).id;
    ...
    if (claimsStructChanged)
        changed.push("claims");
    const statusesUpgradesInvalid = [];
    const statusesUpgradesValid = [];
    ...
    for (const cid of bClaimKeys) {
        ...
        if (strengthNew > strengthOld) {
            ...
            if (newEids.length === 0) {
                statusesUpgradesInvalid.push(`${cid}: ${lvl} ${sOld}→${sNew} (strength ${strengthOld}→${strengthNew}) TANPA EvidenceId BARU (ΔEvidenceIdentity=0 — violates Δ(Status)⇒Δ(Evidence))`);
            }
            else {
                statusesUpgradesValid.push(`${cid}: ${lvl} ${sOld}→${sNew} OK — ΔEvidenceIdentity=${newEids.length} evidence IDs baru`);
            }
        }
    }
    ...
    const relsEq = ra.length === rb.length && ra.every((x, i) => x.f === rb[i].f && x.k === rb[i].k && x.t === rb[i].t && x.id === rb[i].id);
    if (!relsEq)
        changed.push("claimRelations");
    if (String(oldEnv.graphTopology.id) !== String(newEnv.graphTopology.id))
        changed.push("topology");
    if (oldEnv.milestone !== newEnv.milestone ||
...
function computeFullRevocationCascade(env, evidenceId) {
    const base = evidenceRevocationImpact(env, evidenceId);
    const impactedRelationIds = [];
    const affectedSet = new Set(base.affectedSubtreeClaimIds);
    ...
    for (const cid of base.affectedSubtreeClaimIds) {
        ...
        if (!c)
            continue;
        if (c.evidenceLevel === "Architectural" && (c.status === "Supported" || c.status === "Refuted"))
            statusAffected.push(cid);
        if (c.evidenceLevel === "Execution" && c.status === "PASS")
            statusAffected.push(cid);
        if (c.evidenceLevel === "Evolutionary" && (c.status === "Verified" || c.status === "Running"))
            statusAffected.push(cid);
    }
    return Object.freeze({
        revokedEvidenceId: evidenceId,
