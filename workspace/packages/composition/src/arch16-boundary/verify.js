...
const CONSTITUTION = {
    compilerProduces: ["ResolvedWorkspace"],
    runtimeOnlyConsumes: ["ResolvedWorkspace"],
...
function detectStageFromSymbol(symbolName) {
    const n = symbolName.toLowerCase();
    if (n.startsWith("normalize") || n.includes("normalizedworkspace"))
        return "normalization";
    if (n.startsWith("buildcompositionplan") || n.includes("compositionplan"))
        return "planning";
    if (n.startsWith("buildgraph") || n.includes("workspacegraph"))
        return "graph-construction";
    if (n.includes("descriptor") || n.includes("workspaceid") || n.includes("workspacedescriptor"))
...
const verifyArch16 = function verifyArch16(target, options) {
    ...
    const mutableNodes = collectGraph ? {} : undefined;
    if (collectGraph && mutableNodes !== undefined) {
        for (const p of target.runtimeModulePaths)
            mutableNodes[p] = [];
        for (const p of target.compilerModulePaths)
            mutableNodes[p] = [];
    }
    ...
    const hasForbiddenEdge = (runtimePath, compilerPath) => {
        const normalized = `${runtimePath}->${compilerPath}`.toLowerCase();
        return (normalized.includes("normalizer") ||
    ...
    const anyStageFound = Object.values(stagesFound).some(Boolean);
    ...
    if (collectGraph && mutableNodes !== undefined) {
        ...
        for (const k of Object.keys(mutableNodes))
            out[k] = Object.freeze(mutableNodes[k].slice());
...
exports.verifyArch16 = verifyArch16;
