...
function hydrateArticles() {
    const s = new Map();
    for (const c of seedArticles())
        s.set(c.id, c);
    return s;
}
function hydrateDiscussions() {
    const s = new Map();
    for (const d of seedDiscussions())
        s.set(d.id, d);
    return s;
}
const TOPIC_STORE = hydrateTopics();
const ARTICLE_STORE = hydrateArticles();
const DISCUSSION_STORE = hydrateDiscussions();
...
export const ContentArticleRepositoryInMemory = {
    ...
    save(entity) {
        const e = { ...cloneArticle(entity), updatedAt: new Date() };
