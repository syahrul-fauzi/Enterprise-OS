...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json(constitutionGatewayService.getProofBundle());
  } catch (rawError) {
    const detail = rawError instanceof Error ? rawError.message : String(rawError);
    return NextResponse.json({ error: "constitution_proof_bundle_unavailable", detail }, { status: 503 });
  }
}
