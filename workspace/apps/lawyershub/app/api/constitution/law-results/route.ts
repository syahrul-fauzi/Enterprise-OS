...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json(constitutionGatewayService.getLawResults());
  } catch (rawError) {
    const detail = rawError instanceof Error ? rawError.message : String(rawError);
    return NextResponse.json({ error: "constitution_law_results_unavailable", detail }, { status: 503 });
  }
}
