...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json(constitutionGatewayService.getCertificates());
  } catch (rawError) {
    const detail = rawError instanceof Error ? rawError.message : String(rawError);
    return NextResponse.json({ error: "constitution_certificates_unavailable", detail }, { status: 503 });
  }
}
