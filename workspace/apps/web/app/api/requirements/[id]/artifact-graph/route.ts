...
export async function GET(_request: Request, segment: { params: Params }) {
  try {
    ...
    // Return only artifact graph related data (if exists on requirement)
    return NextResponse.json({
      artifactGraph: output.linkedCapabilityIds || [],
    }, { status: 200 });
  } catch (error) {
    console.error("[GET /api/requirements/[id]/artifact-graph] Error:", error);
    const errorMessage = error instanceof Error ? error.message : "Failed to fetch artifact graph";
