...
// import { ProductPeoplePage } from "@repo/presentation-widgets"; // Deprecated package - golden spine only
...
export async function generateMetadata({ params }: PeopleRouteProps): Promise<Metadata> {
...
export default async function PeopleRoute({ params, searchParams }: PeopleRouteProps) {
  // Resolve session FIRST - canonical pattern from golden spine
  const session = await resolveSessionOrEnter();
  const { productId = "lawyershub" } = await params;
  const sp = await searchParams;
  const isNewPerson = sp.new === "person";
  const binding = readProductBinding(productId);
  return <ProductPeoplePage productId={productId} binding={binding} session={session} isNewPerson={isNewPerson} />;
}