...
import type { VerificationProofObject } from "../lib/proof-object";
...
export function RequirementProofPanel({ productId, requirementId }: RequirementProofPanelProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <h2 className="text-xl font-bold text-slate-900 mb-4">5. Verdict-nya apa? (What is the verdict?)</h2>
        <div className="grid gap-4 md:grid-cols-3">
          <div className={`rounded-2xl border-2 p-6 ${
            proof.proven ? "border-emerald-300 bg-emerald-50" : "border-amber-300 bg-amber-50"
          }`}>
            <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Status</div>
            <p className={`mt-2 text-2xl font-bold ${proof.proven ? "text-emerald-700" : "text-amber-700"}`}>
              {proof.verdict.status.toUpperCase()}
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
            <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Confidence</div>
            <p className="mt-2 text-2xl font-bold text-slate-900">{(proof.verdict.confidence * 100).toFixed(0)}%</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
            <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Decision Fingerprint</div>
            <p className="mt-2 text-sm font-mono text-slate-700 truncate">{proof.verdict.summary}</p>
          </div>
        </div>
      </section>

      <section className={`rounded-3xl border-2 p-6 shadow-sm sm:p-8 ${
        proof.proven ? "border-emerald-300 bg-emerald-50" : "border-amber-300 bg-amber-50"
      }`}>
        <h2 className="text-xl font-bold text-slate-900 mb-4">6. Apakah requirement tersebut benar-benar proven? (Is this requirement truly proven?)</h2>
        <div className="flex items-center gap-4">
          <span className={`text-6xl ${proof.proven ? "text-emerald-600" : "text-amber-600"}`}>
            {proof.proven ? "✅" : "⚠️"}
          </span>
          <div>
            <p className={`text-2xl font-bold ${proof.proven ? "text-emerald-700" : "text-amber-700"}`}>
              {proof.proven ? "YES - This requirement is FULLY PROVEN" : "NO - Additional evidence required"}
            </p>
            <p className="mt-1 text-slate-600">
              {proof.proven 
                ? "All traceability links are complete, evidence has been collected, and the governance verdict is PASS."
                : "Some traceability or evidence artifacts are missing. The requirement cannot be considered proven yet."}
            </p>
          </div>
        </div>
      </section>
...
export default RequirementProofPanel;